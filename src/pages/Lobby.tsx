...
interface Player { id: string; name: string; ready: boolean; spectator: boolean; alive: boolean; }
...
<span>{p.name}</span>
<span className={`text-sm ${p.ready?"text-green-600":"text-gray-500"}`}>{p.ready?"Ready":"Not ready"}</span>
</li>
))}
</ul>
<button className="btn" onClick={toggleReady}>준비 완료</button>
{allReady && <p className="text-primary">모두 준비 완료! 곧 시작합니다…</p>}
</div>
);
}